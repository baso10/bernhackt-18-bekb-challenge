...
import { Divider, List, ListItem, Tab, Tabs, TextField } from '../node_modules/@material-ui/core';
...
class Orderscreen extends Component {
  ...
  logout = () => {this.props.appContext.setState({token: null})}
  ...
  clearOrderList = () => {this.setState({productOrder: []})}
  ...
  _handleChange = (newValue, actionMeta) => {
    ...
    if(newValue.length != 0){
      ...
      orderInput(newProduct,'Menge für ','Anzahl | Grösse | Gewicht').then((result) => {
        if (result.value) {
      ...
      newProduct.name = input.value;
      var tempList = this.state.productOrder;

      Swal({
        title: 'Menge für ' + newProduct.name,
        text: 'Anzahl | Grösse | Gewicht',
        input:'text',
        showCancelButton: true,
        confirmButtonText: 'Hinzufügen',
        cancelButtonText: 'Abbrechen'
  ...
  productListRender = () => {
   const that = this;
      ...
      return (
        this.state.productOrder.map(function(product, index){
          return <ListItem key={index}>
                    <TextField type="text" value={product.count}/>
                    <TextField type="text" value={product.name} style={{width:"50em",marginLeft:20, paddingRight:100}}/>
                    <Button
                        id={index} 
                        variant="fab" 
                        aria-label="Delete"
          ...
          return <ListItem>Noch keine Produkte auf der nächsten Bestellung....</ListItem>;
        }
  }
      ...
      return (
          <MuiThemeProvider>
            <div style={{display:"inline-block", width:"90%"}}>
              ...
              <div id="bottomBar">
              <Button color="secondary" variant="outlined" style={style} onClick={this.clearOrderList}>Bestellung zurücksetzten</Button>
              <Button color="primary" variant="outlined" style={style} onClick={this.sendOrder}>Bestellung abschicken</Button>
              </div>
            </div>
          </MuiThemeProvider>
      );  
